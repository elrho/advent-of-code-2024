...
class aoc5Test {
    ...
    @Test
    fun `page update is in order`() {
        val update = listOf(75, 47, 61, 53, 29)

        val expected = true

        assertThat(isOrdered(pageOrders, update)).isEqualTo(expected)
    }

    @Test
    fun `page update is not in order`() {
    ...
    @Test
    fun `turn list into map`() {
        val expected = mapOf<Int, Int>()

        assertThat(mapPageOrders(pageOrders)).isEqualTo(expected)
    }

    @Test
    fun `turn updates into list`() {
        val expected = listOf(75, 47, 61, 53, 29)

        assertThat(createUpdateList(updates)).isEqualTo(expected)
    }

    @Test
    fun `find all number of ordered lists`() {
        val expected = 143

        assertThat(countOrders(inputOrder, inputUpdates)).isEqualTo(expected)
    }

    @Test
    fun `part 2 - correct unordered lists`() {

        val expected = listOf(97, 75, 47, 29, 13)

        val input = listOf(97, 13, 75, 29, 47)

        assertThat(orderRuleBroken(pageOrders, input)).isEqualTo(expected)
    }

    @Test
    fun `part 2 - correct unordered lists 2`() {

        val input = listOf(75,97,47,61,53)

        val expected = listOf(97,75,47,61,53)

        assertThat(orderRuleBroken(pageOrders, input)).isEqualTo(expected)
    }

    @Test
    fun `part 2 - correct unordered lists 3`() {

        val input = listOf(61,13,29)

        val expected = listOf(61,29,13)

        assertThat(orderRuleBroken(pageOrders, input)).isEqualTo(expected)
    }

    @Test
    fun `part 2 - find sum of middles for all incorrect orders`() {

        val expected = 0

        assertThat(correctedOrdersTotal(pageOrders, updates)).isEqualTo(expected)
    }

    @Test
    fun `part -2 return not ordered lists`() {
        val lists = createUpdateList(inputUpdates)

        println(lists)

        val incorrect = lists.filter { !isOrdered(inputOrder, it) }

        println(incorrect)

        val expected = listOf<Int>()

        assertThat(incorrect).isEqualTo(expected)
    }
